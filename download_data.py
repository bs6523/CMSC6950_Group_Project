...
def download(stationid, year):
    ...
    if(os.path.exists(File_name)): #download only if the file not Exists
        print("File Exists")
    else:
    ...
    data = pd.read_csv(File_name, skiprows=25, sep=",", encoding="ISO-8859-1")
    ...
    Citi_name = pd.read_csv(File_name, nrows=1)
